...
public class FileUtil {
    ...
    public static String saveBmp2Gallery(Context context, Bitmap bitmap, String pictureName) {
        if (Build.VERSION.SDK_INT >= 29) {
            return saveSignImage(context, pictureName + ".jpg", bitmap);
        } else {
            ...
            try {
                File path = new File(galleryPath);
                if (!path.exists()) {
                    path.mkdirs();
                }
                // 如果有目标文件，直接获得文件对象，否则创建一个以filename为名称的文件
                file = new File(galleryPath, pictureName + ".jpg");
                // 获得文件相对路径
    ...
    //将文件保存到公共的媒体文件夹
    public static String saveSignImage(Context context, String fileName, Bitmap bitmap) {
        try {
